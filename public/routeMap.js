...
// The four columns span 320 units; the rest is margin for the row-change
// connector, which bulges sideways past the outer columns (see ROW_BULGE).
// X_OFF shifts the columns right so that margin is symmetric.
const CANVAS_W = 336;
const X_OFF = 8;
...
// cy-40.6, and SVG text grows upward from its baseline (~7 units of ascender at
// font-size 9), so the glyphs reach cy-47.6.
const TOP_PAD = 52;
...
//
// The ratios are what make it read as a map pin rather than a balloon on a
// stick: the tip sits 1.8 balloon-radii below the balloon centre, and the hole
// is 0.45 of the radius. A taller pin (this was 2.55 radii) looks lean and
// needle-like, and a smaller hole loses the punched-out look entirely.
const PIN_R = 12;
const PIN_H = PIN_R * 1.8;
const PIN_HOLE = PIN_R * 0.45;

// Control-point offset for the row-change connector. A single cubic bulges
// 0.75*k from the chord, so k = 59 gives 44.25 -- half of ROW_H, which is
// exactly a semicircle. It reaches x=335.25 on the right and 0.75 on the left,
// which is why CANVAS_W is 336 rather than the 320 the columns need.
const ROW_BULGE = 59;
...
function serpentineLayout(waypoints) {
  ...
  // (cy+26, plus descender and a little air), not a whole extra row -- a
  // one-row map should not carry 88 units of dead space beneath it.
  return { nodes, width: CANVAS_W, height: (rows - 1) * ROW_H + TOP_PAD + 32 };
}
...
// A map pin: tip at the anchor, balloon centred PIN_H above it.
//
// Each side starts at the balloon's WIDEST point and leaves straight down. The
// circle's tangent is vertical there, so the join is seamless and the balloon's
// full width carries into the body before it tapers -- that is what gives the
// pin its belly. Running the sides to a tangent point partway up the balloon
// instead (the obvious construction) pinches the body in and reads as lean.
// The control points sit a third and two thirds of the way down, the second
// pulled halfway in toward the axis.
function pinPath(cx, cy) {
  const by = cy - PIN_H;
  const third = cy - PIN_H / 3;
  const twoThirds = cy - (PIN_H * 2) / 3;
  return [
    `M${cx} ${cy}`,
    `C${cx - PIN_R / 2} ${third}, ${cx - PIN_R} ${twoThirds}, ${cx - PIN_R} ${by}`,
    `A${PIN_R} ${PIN_R} 0 0 1 ${cx + PIN_R} ${by}`,
    `C${cx + PIN_R} ${twoThirds}, ${cx + PIN_R / 2} ${third}, ${cx} ${cy}`,
    'Z',
  ].join(' ');
}

// The pin is filled with its category colour and a hole is punched through the
// balloon, rather than being outlined in that colour. The colour gets far more
// area this way while the pin stays visually light, and nothing has to sit
// legibly on top of it -- the visit number that used to live there is gone,
// since the route line already shows what connects to what.
//
// The hole's fill (--paper) comes from CSS on .rm-hole, never a `fill="..."`
// attribute: `var()` is not a valid paint value in SVG's presentation attribute
// grammar, so an attribute would be dropped and fall back to black.
//
...
function renderNode(node, showDate) {
  ...
  return `<g>
    ...
    ${showDate ? `<text class="rm-date" x="${cx}" y="${cy - PIN_H - PIN_R - 7}">${escapeHtml(formatDate(w.date))}</text>` : ''}
    <path class="rm-node" d="${pinPath(cx, cy)}" fill="${categoryMark(w.category)}"></path>
    <circle class="rm-hole" cx="${cx}" cy="${cy - PIN_H}" r="${PIN_HOLE}"></circle>
    <text class="rm-label" y="${cy + 15}">${label}</text>
  </g>`;
}
