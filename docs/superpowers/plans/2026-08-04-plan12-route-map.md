...
**스펙 §2.3에 대한 정정 — 그리고 그 정정이 틀렸다는 재정정:**

이 계획서는 원래 "행이 바뀌어도 열 슬롯이 그대로라 `cx`가 같으니 베지어가 직선으로 퇴화한다. 수직 직선으로 그린다"고 지시했다. **이는 잘못이었다.** 최종 리뷰에서 잡혔고 `3a672a2`에서 되돌렸다.

`cx`가 같다는 관찰 자체는 맞다. 놓친 것은 **곡선이 바깥으로 부푸는 이유**였다 — 그 자리에 윗 노드의 장소명과 아랫 노드의 날짜 칩이 있다. 2→3 경계에서 실측하면:

```
연결선: x=250, y 59 → 121
윗 노드 라벨 baseline: y=72, y=83  (x=250)
아랫 노드 날짜 칩:     y=115       (x=250)
```

직선으로 그으면 매 행 경계마다 장소명과 날짜를 2px 선이 관통한다. 스펙이 곡선을 지정한 건 미관이 아니라 텍스트를 피해 가기 위해서였다.

**따라서 3차 베지어로 그린다.** 제어점을 캔버스 중앙 반대쪽으로 34단위 밀면 최대 25.5단위 벗어나 텍스트(반폭 약 20단위)를 비껴가고, x=275.5로 300폭 안에 머문다. 방향은 `cx > CANVAS_W / 2`로 판정해 양쪽 가장자리에서 모두 동작해야 한다.
